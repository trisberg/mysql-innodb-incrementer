...
/**
 * {@link org.springframework.jdbc.support.incrementer.DataFieldMaxValueIncrementer} that
 * increments the maximum value of a given MySQL table
 ...
 * can be INNODB since the sequences are allocated using a separate connection without
 * regard to any transactions that might be in progress.
 *
 * <p>Example:
 *
 * <pre class="code">create table tab (id int unsigned not null primary key, text varchar(100));
 * create table tab_sequence (value int not null) engine=INNODB;
 ...
 */
public class MySQLInnoDbMaxValueIncrementer extends AbstractColumnMaxValueIncrementer {
	 ...
	 */
	public MySQLInnoDbMaxValueIncrementer(DataSource dataSource, String incrementerName, String columnName) {
	...
	protected synchronized long getNextKey() throws DataAccessException {
		if (this.maxId == this.nextId) {
			/*
			* We obtain a non-managed connection so our modifications are handled as a separate transaction.
			* We also use straight JDBC code because we need to make sure that the insert and select
